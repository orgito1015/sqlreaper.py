...
from utils.validator import clamp_level, clamp_risk, clamp_threads
...
def build_config(args: Any) -> ScanConfig:
    ...
    # Clamp numeric ranges regardless of whether they came from a profile or CLI
    cfg.level = clamp_level(cfg.level)
    cfg.risk = clamp_risk(cfg.risk)
    cfg.threads = clamp_threads(cfg.threads)
