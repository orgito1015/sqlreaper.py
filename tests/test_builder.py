...
def test_tamper_injected_when_set(basic_cfg):
    """A profile's --tamper value must reach the actual sqlmap flags."""
    basic_cfg.tamper = "space2comment,randomcase"
    flags = base_flags(basic_cfg)
    assert "--tamper=space2comment,randomcase" in flags


def test_tamper_absent_when_not_set(basic_cfg):
    """--tamper flag should not appear when cfg.tamper is empty."""
    flags = base_flags(basic_cfg)
    assert not any(f.startswith("--tamper=") for f in flags)


def test_delay_injected_when_set(basic_cfg):
    """A profile's delay value must reach the actual sqlmap flags."""
    basic_cfg.delay = 2
    flags = base_flags(basic_cfg)
    assert "--delay=2" in flags


def test_delay_absent_when_zero(basic_cfg):
    """--delay flag should not appear when cfg.delay is 0."""
    flags = base_flags(basic_cfg)
    assert not any(f.startswith("--delay=") for f in flags)
