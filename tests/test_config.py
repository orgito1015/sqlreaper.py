...
class TestBuildConfig:
    ...
    def test_stealth_profile_sets_tamper(self):
        args = self._make_args(profile="stealth")
        cfg = build_config(args)
        assert cfg.tamper == "space2comment,randomcase"

    def test_stealth_profile_sets_delay(self):
        args = self._make_args(profile="stealth")
        cfg = build_config(args)
        assert cfg.delay == 2

    def test_out_of_range_level_is_clamped(self):
        args = self._make_args(level=99)
        cfg = build_config(args)
        assert cfg.level == 5

    def test_out_of_range_risk_is_clamped(self):
        args = self._make_args(risk=-3)
        cfg = build_config(args)
        assert cfg.risk == 1

    def test_out_of_range_threads_is_clamped(self):
        args = self._make_args(threads=500)
        cfg = build_config(args)
